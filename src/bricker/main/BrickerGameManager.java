...
//        this.lives=3;
    ...
    private void createObjects(ImageReader imageReader, SoundReader soundReader) {
        ...
        StrategyFactory strategyFactory = new StrategyFactory(gameObjects(), bricksCounter, imageReader,
                soundReader, BALL_SPEED, puckList, BALL_RADIUS, heartDimentions, lives,
                HEART_IMG_PATH, inputListener, windowDimentions, BORDER_WIDTH,
                createPaddleDimension(), PADDLE_IMG_PATH, collisionCounter, ball, heartList,this);
    ...
    private void checkForResetCamera() {
        if (ChangeCameraStrategy.getCollisionCounterInHit() + BALL_HITS_TO_RESET_CAMERA +
                HIT_OFFSET == ball.getCollisionCounter()) {
            this.setCamera(null);
        }
    }
    ...
    private void createNumericCounter() {
        ...
        Vector2 topLeftCorner = new Vector2(BORDER_WIDTH, windowDimentions.y() - 2 * (BASIC_SPACE + PADDLE_HEIGHT));
        NumericLifeCounter numericLifeCounter = new NumericLifeCounter(topLeftCorner,
                dimentions, lives, gameObjects());
    ...
    private void createBrick(Renderable brickImage, Vector2 brickVector, Vector2 windowDimentions,
        ...
        Brick brick = new Brick(brickVector, windowDimentions, brickImage, strategyFactory.generateStrategies());
